...
import { IQuestionNode } from "../../../../types.ts";
...
describe("QuantityNode unitOption extension", () => {
  ...
  it("auto selects the single unit option", async () => {
    ...
    const { getByRole } = render(<Node node={question} />);

    const combobox = getByRole("combobox");
    expect(combobox).toHaveValue("http://unitsofmeasure.org::ml::mL");
  ...
  it("applies the single unit once the user types a value", async () => {
    ...
    expect(combobox).toHaveValue("http://unitsofmeasure.org::Cel::°C");
  });
  ...
  it("shows a disabled option for legacy units until a new selection is made", async () => {
    ...
    const { getByRole, queryByRole } = render(<Node node={question} />);
    ...
    await user.selectOptions(
      combobox,
      "http://unitsofmeasure.org::mL/h::mL/hour",
    );

    expect(queryByRole("option", { name: "mL/day", hidden: true })).toBeNull();
    expect(getByRole("option", { name: "mL/hour" })).toHaveProperty(
      "selected",
      true,
    );
  });
