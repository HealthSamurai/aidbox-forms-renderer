...
import { RepeatingGroupScaffold } from "../repeating-group-scaffold.tsx";

export const GTableControl = function GTableControl({
  wrapper,
}: RepeatingGroupControlProps) {
  return (
    <RepeatingGroupScaffold wrapper={wrapper}>
      <RepeatingGroupMatrix wrapper={wrapper} />
    </RepeatingGroupScaffold>
  );
};
...
const RepeatingGroupMatrix = observer(function RepeatingGroupMatrix({
  ...
  const columns = buildColumnSpecs(wrapper, wrapper.visibleNodes);
            ...
            wrapper.visibleNodes.map((node, index) => {
              ...
              return (
                <Fragment key={node.key}>
                  <tr>
                    <th scope="row">{getNodeLabel(wrapper, node, index)}</th>
                    {columns.map((column) => (
...
function buildColumnSpecs(
  ...
  (wrapper.template.item ?? []).forEach((item) => {
...
function getNodeLabel(
  ...
  const base = node.template.text ?? wrapper.template.text ?? "Entry";
  return `${base} #${index + 1}`;
}
